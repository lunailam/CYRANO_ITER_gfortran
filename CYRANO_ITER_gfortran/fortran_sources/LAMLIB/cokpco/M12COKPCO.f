...
c	Variables
	...
	character(100) :: GGs, GG2s, GGstot, GG2stot   
	character(2) :: charaux, charaux2,charauxtot, charaux2tot
...
c	    6) Species (and radius) dependent factor (p = +-1):

c	    NB: The M12cokpco_write/read routines return the following quantity:
c			M12 = -i.A.sum(Pell x Tell), with A = p.wcdelta / |k//.vT|

c		- Geometrical factor from paper (polariz. factor 'p' dealt with later):
c		  (division by wcdelta to cancel with 'A' factor)	     
	    factor = eps0 * dPsidr_n(intab)*abscis(intab) / hachi(intab) *
     ;             wplasma(ispec)**2 /  wcdelta(ispec) 

c		- Additional factors: (-ci): CYRANO convention (different from paper)
c							   1/b0: Conversion from gcdr do Pell coefficients
...
c	  NB: (-) sign comes from the 'p' factor embedded in definition of A.

cERN	NEW: Write tootal matrix

	if (WRITE_OUTPUT) then

	        call INT_TO_STRING(2*NM+1, charauxtot)
	        call INT_TO_STRING(2*NM+2, charaux2tot)
	             GGstot  = "(G11.5," // charauxtot // "G20.8)"
	             GG2stot = "(" // charaux2tot // "G20.8)"

         FILE_NAME = COKFOLDER  // "/M12" // 'total' // polname(1)
			open (UNIT = 13, FILE = FILE_NAME, STATUS = "REPLACE",
c     ;			  BUFFERED='YES', BLOCKSIZE = 32*512, BUFFERCOUNT = 50, 
     ;              IOSTAT = OpenStat, ACTION = "WRITE")
	              if (OpenStat > 0) then
				  ...
				  write (13, GGstot), '0.0', '0.0', mdiffaux(1:2*NM)
				  do k = 1, NA
					 write (13, GG2stot),  rmaux(k), allkpa(k), m12left(k,1:NM)
				  end do 
			close (13)	    

         FILE_NAME = COKFOLDER  // "/M12" // 'total' // polname(2)
			open (UNIT = 13, FILE = FILE_NAME, STATUS = "REPLACE",
c     ;			  BUFFERED='YES', BLOCKSIZE = 32*512, BUFFERCOUNT = 50, 
     ;              IOSTAT = OpenStat, ACTION = "WRITE")
	              if (OpenStat > 0) then
				  ...
				  write (13, GGstot), '0.0', '0.0', mdiffaux(1:2*NM)
				  do k = 1, NA
					 write (13, GG2stot), rmaux(k), allkpa(k), m12right(k,1:NM)
				  end do 
			close (13)		
...
c	    8) Species (and radius) dependent factor (p = 0):

c	    NB: The M12cokpco_landau routine returns the following quantity:
c			M12 = -i.Pell.(z/w).I2(z), with I2(z) = z.(1+z.ZFC(z)) and z = w/|k//.vT|

c		- Geometrical factor from paper 	
	    factor = eps0 * dPsidr_n(intab)*abscis(intab) / hachi(intab) *
     ;             wplasma(ispec)**2 

c		- Additional factors: (-ci): CYRANO convention (different from paper)
c							   1/b0: Conversion from gcdr to Pell coefficients
cERNccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c		  ------->  EXTRA FACTOR : 2*pi
cERNccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	...
	if (WRITE_OUTPUT) then
         ...
         FILE_NAME = COKFOLDER  // "/M12" // "total_landau.dat"
			open (UNIT = 13, FILE = FILE_NAME, STATUS = "REPLACE",
c     ;			  BUFFERED='YES', BLOCKSIZE = 32*512, BUFFERCOUNT = 50, 
     ;              IOSTAT = OpenStat, ACTION = "WRITE")
	              if (OpenStat > 0) then
				  ...
				  write (13, GGstot), '0.0','0.0', mdiffaux(1:2*NM)
				  do k = 1, NZ
					 write (13, GG2stot),  rmaux(k), allkpa(k), m12landau(k,1:NM)
				  end do 
			close (13)
...
c	  (2) Fill the missing (negative) m1-m2 terms according to symmetrie (even)
		  do j = 1, NM-1
			 m12left  (1:NA, j) = m12left  (1:NA, NM+klim-j+1)
			 m12right (1:NA, j) = m12right (1:NA, NM+klim-j+1)
			 m12landau(1:NA, j) = m12landau(1:NA, NM+klim-j+1)
		  end do
