...
c     1.2) Z coordinate (closed surfaces)
	open(UNIT = 99, FILE = '../../Profiles/Zflush.dat', STATUS = "OLD",
     ;     IOSTAT = OpenStat, ACTION = "READ")
...
c     1.1) B toroidal (closed surfaces)
	open(UNIT = 99, FILE = '../../Profiles/Btorflush.dat', STATUS = "OLD",
     ;     IOSTAT = OpenStat, ACTION = "READ")
	     read ( 99, *), rhoB_flu(1:Nflu)
		 do j = 1,1000
		     read ( 99, *, END = 666), Btor_flu(1:Nflu,j)
		 end do
	close (99)
666	continue

c     1.2) Pitch angle (closed surfaces)
	open(UNIT = 99, FILE = '../../Profiles/Pitchflush.dat', STATUS = "OLD",
     ;     IOSTAT = OpenStat, ACTION = "READ")
	     read ( 99, *), rhoP_flu(1:Nflu)
		 do j = 1,1000
		     read ( 99, *, END = 777), Pitch_flu(1:Nflu,j)
		 end do
	close (99)
777	continue

c     1.3) Safety factor (closed surfaces)
	open(UNIT = 99, FILE = '../../Profiles/qprofflush.dat', STATUS = "OLD",
     ;     IOSTAT = OpenStat, ACTION = "READ")
	     read ( 99, *), rhoQ_flu(1:Nflu)
	     read ( 99, *), Q_flu(1:Nflu)
	close (99)
...
c     1.4) B toroidal (closed surfaces)
	rhoB_flu(Nflu+1) = (rhoR_flu(Nflu) + rho_wall) / 2
	rhoB_flu(Nflu+2) = rho_wall

	Btor_flu(Nflu+1,:) = Btor_flu(Nflu,:)
	Btor_flu(Nflu+2,:) = Btor_flu(Nflu,:)

	Pitch_flu(Nflu+1,:) = Pitch_flu(Nflu,:)
	Pitch_flu(Nflu+2,:) = Pitch_flu(Nflu,:)

c	Radial interpolation to Cyrano abscissa 
	do j = 1, 257
		call spline0(Nflu+2, rhoB_flu,       Btor_flu(1:Nflu+2,j), 
     ;                 i_rw,   abscno(1:i_rw), Btor_cyr(1:i_rw,j))
		call spline0(Nflu+2, rhoB_flu,       Pitch_flu(1:Nflu+2,j), 
     ;                 i_rw,   abscno(1:i_rw), Pitch_cyr(1:i_rw,j))
	end do
