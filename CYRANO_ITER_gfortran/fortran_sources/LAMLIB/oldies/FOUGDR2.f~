...
c     NB: Thbar was tabulated vs theta in ckt(,,3), khi in ckt(,,5).
      call interp2(ckt(intab,1,5), nabplo, ckt(intab,1,3), nabplo, npp, polang, thbmkh, npp)
c	open(unit=26, file='..\..\thbmkh', status='unknown')
        do ipo = 1, npp
        thbmkh(ipo) = thbmkh(ipo) - polang(ipo)
c	  write(26,*)ipo, thbmkh(ipo)
        end do
...
c      if(icase.eq.1)
c     ;call interp2(ckt(intab,1,5), nabplo, bmotab(intab,1), nabplo, npp
c     ;, polang, bvskhi, npp)

c     Get dkhi/dtheta by Fourier tf:
        do ipo = 1, npfft
	  cr(ipo,0) = (ckt(intab,ipo,5) - polang(ipo)) * rnfi
	  end do

cccccccccccccccccccccccccc ERNESTO ccccccccccccccccccccccccccccccc
...
cERN		call cfft2(0, 1, npfft, cr(1,0), cworkm, cr(1,0))
	    ...
	    do ipo = 1, npfft
	       dkhdth(ipo) = dreal(cr(ipo,0)) + 1.d0
	    end do
cERN	Impose last point (2*pi) is equal to first point (0)
		dkhdth(npfft+1) = dkhdth(1) 
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

c     Table of dThetabar/dkhi on standard theta mesh:
      dthdkh(1) = (ckt(intab,2,3)-ckt(intab,1,3)) /
     ;            (ckt(intab,2,5)-ckt(intab,1,5))
        do ipo = 2, npp-1
        dthdkh(ipo) =
     ;     (ckt(intab,ipo+1,3)-ckt(intab,ipo-1,3)) /
     ;     (ckt(intab,ipo+1,5)-ckt(intab,ipo-1,5))
        end do
      dthdkh(npp) = 
     ;     (ckt(intab,npp,3)-ckt(intab,npp-1,3)) /
     ;     (ckt(intab,npp,5)-ckt(intab,npp-1,5))

	  do ipo = 1, npp
	  dthdkh(ipo) = ckt(intab,ipo,1) / dkhdth(ipo)
	  end do

c     Additional factor for Maxwellian case:
        if(icase.eq.1)then
	    do ipo = 1, npp
	    dthdkh(ipo) = dthdkh(ipo)*b0/bmotab(intab,ipo)
	    end do
	  end if

c     Interpolate to uniform khi mesh:
      call interp2(ckt(intab,1,5), nabplo, dthdkh, 1, npp, polang, dtdkvk, npp)

c     In cr(*,k), store exp{ik(Thbar-khi)} dThbar/dkhi / npfft
c     on equidistant khi mesh:
c        if(icase.eq.1)then
cc       Additional factor for Maxwellian case:
c	    do ipo = 1, npp
c	    cr(ipo,0) = dcmplx(dtdkvk(ipo)*b0/bvskhi(ipo)*rnfi, 0.d0)
c	    end do
c	  else
	    do ipo = 1, npp
	    cr(ipo,0) = dcmplx(dtdkvk(ipo)*rnfi, 0.d0)
	    end do
c	  end if

        do ipo = 1, npp
c       Exponential of i*(thbar-khi):
        ei(ipo) = cdeid(thbmkh(ipo))
...
c	write(6,*)'Fougdr2: npfft=', npfft
c      print *, cr(1:10,0)
c	print *, 'Stop at FOUGDR'
c	stop
...
c     NB: Thbar was tabulated in ckt(,,3), khi in ckt(,,5).
      call interp2(ckt(intab,1,3), nabplo, ckt(intab,1,5), nabplo, npp, polang, khothb, npp)

c     For Maxwellian case: different geom. coeffs, B0 required:
      if(icase.eq.1)
     ;call interp2(ckt(intab,1,3), nabplo, bmotab(intab,1), nabplo, npp, polang, bvsthb, npp)
...
cERN  ccccccccccccccc Writing OUTPUT file ccccccccccccccccccccccccc
	...
	if (WRITE_TABLES) then
     ...
     ;		  STATUS = "REPLACE", ACTION = "WRITE")
		write(777,*)'Geometrical Fourier coef. for M12 response:'
		do ka = 0, klim
		   write(777,*)'ka=',ka
