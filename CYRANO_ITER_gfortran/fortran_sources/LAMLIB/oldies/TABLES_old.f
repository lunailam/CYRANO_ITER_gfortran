...
c
     ...
     ;, jnor(npfft+1), jnorav
      ...
      external inscr, gauss, totcun, g, idamin, idamax, i1st, nsum
...
c     11.3) Thbar and Phibar, using trapeze integration: 
		  do ipo = 2, npp
			 ckt(i,ipo,3) = ckt(i,ipo-1,3) + 
     ;		 polst * 0.5 * (ckt(i,ipo-1,1) + ckt(i,ipo,1))
			 ckt(i,ipo,4) = ckt(i,ipo-1,4) + 
     ;		 polst * 0.5 * (ckt(i,ipo-1,2) + ckt(i,ipo,2))
		  end do
		  if(updsym)then
			 ckt(i,npp,3) = pi
		  else
...
c		    TABLES.f (and in its subroutines DSHAP2.f)

	qfactor(1:nabsci)    = eqta1d(1:nabsci,4)	! Safety factor q(r)
c	used in: OUTPOW, FOUCOG2, FOUCOJ, M12COKPCO, OUTDIS (only save)  
cERN	Psir(1:nabsci)       = eqta1d(1:nabsci,7)	! Poloidal flux Psi(r)
c	used in: OUTDIS (only save) ---> removed from OUTDIS (now in TABLES)
	dPsidr_n(1:nabsci)   = eqta1d(1:nabsci,6)	! 1/r dPsi/dr
c	used in: DIRESP, POWGDR 
	surf_perim(1:nabsci) = eqta1d(1:nabsci,10)	! Surface perimeter
c	used in: PRESO2 (dynmod = T)
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc


cERNcccccccccccccccccccc Saving TABLES to files ccccccccccccccccccccccccccc
...
c     3.3) Pitch angle - THETA(rho,theta)
	open (UNIT = 777, FILE = paplda // 'pitchangle.dat', 
     ;      STATUS = "REPLACE", ACTION = "WRITE")
		...
		write(777,"(i5, i5, i5)"), nabsci, npfft+1, i_rp 
		do j = 1, nabsci	
		   write(777,2222)(dasin(eqt(j,ipo,15)), ipo = 1, npfft+1)
		end do
	close(777)

c     3.4) Toroidal magnetic field - Btor(rho,theta) = |B|.cos(THETA)
	open (UNIT = 777, FILE = paplda // 'Btor.dat', 
     ;      STATUS = "REPLACE", ACTION = "WRITE")
		...
		write(777,"(i5, i5, i5)"), nabsci, npfft+1, i_rp 
		do j = 1, nabsci	
		   write(777,2222)(bmotab(j,ipo)*eqt(j,ipo,14), ipo = 1, npfft+1)
		end do
	close(777)

c     3.5) Parameter mu = (d2Zdth2*dRdth - d2Rdth2*dZdth) / ntn**2 
...
c		     contains several geometrical coefficients.
	open (UNIT = 777, FILE = paplda // 'mu.dat', 
     ;      STATUS = "REPLACE", ACTION = "WRITE")
		...
		write(777,"(i5, i5, i5)"), nabsci, npfft+1, i_rp 
		do j = 1, nabsci	
		   write(777,2222)(eqt(j,ipo,10), ipo = 1, npfft+1)
		end do
	close(777)

c     3.5) Poloidal metric coefficient N_theta (normalized)
	open (UNIT = 777, FILE = paplda // 'N_theta.dat', 
     ...
     ; = sqrt(drthn*drthn + dzthn*dzthn)'
		...
		write(777, "(i5, i5, i5)"), nabsci, npfft+1, i_rp 
		do j = 1, nabsci	
		   write(777,2222) (eqt(j,ipo,7), ipo = 1, npfft+1)
		end do
	close(777)
...
c		do j = 1, nabsci	
c		   write(777,2222)(eqt(j,ipo,3), ipo = 1, npfft+1)
c		end do
c	close(777)

c     3.7) 1/rho dR/dtheta
...
c		write(777,"(i5, i5, i5)"), nabsci, npfft+1, i_rp 
c		do j = 1, nabsci	
c		   write(777,2222)(eqt(j,ipo,4), ipo = 1, npfft+1)
c		end do
c	close(777)
...
c		write(777,"(i5, i5, i5)"), nabsci, npfft+1, i_rp 
c		do j = 1, nabsci	
c		   write(777,2222)(eqt(j,ipo,5), ipo = 1, npfft+1)
c		end do
c	close(777)

c     3.9) 1/rho dZ/dtheta
...
c		write(777,"(i5, i5, i5)"), nabsci, npfft+1, i_rp 
c		do j = 1, nabsci	
c		   write(777,2222)(eqt(j,ipo,6), ipo = 1, npfft+1)
c		end do
c	close(777)

c     4) 1D COKPCO tables  ---------------------------------------
...
c     5) 2D COKPCO tables  ---------------------------------------

      if(cokpco)then

c     5.1) Magnetic angle Chi 
	open (UNIT = 777, FILE = paplda // 'Chi.dat', 
     ;      STATUS = "REPLACE", ACTION = "WRITE")
		...
		write(777,"(i5, i5, i5)"), nabsci, npfft+1, i_rp 
		do j = 1,nabsci
		   write(777,2222)(ckt(j,ipo,5), ipo = 1, npfft+1)
		end do
	close(777)	

c     5.2) Thetabar coordinate 
	open (UNIT = 777, FILE = paplda // 'Thetabar.dat', 
     ;      STATUS = "REPLACE", ACTION = "WRITE")
		...
		write(777,"(i5, i5, i5)"), nabsci, npfft+1, i_rp 
		do j = 1,nabsci
		   write(777,2222)(ckt(j,ipo,3), ipo = 1, npfft+1)
		end do
	close(777)	

c     5.3) Phibar coordinate 
	open (UNIT = 777, FILE = paplda // 'Phibar.dat', 
     ;      STATUS = "REPLACE", ACTION = "WRITE")
		...
		write(777,"(i5, i5, i5)"), nabsci, npfft+1, i_rp 
		do j = 1,nabsci
		   write(777,2222)(ckt(j,ipo,4), ipo = 1, npfft+1)
		end do
	close(777)	

	end if ! (cokpco)  

c     6) SPECIES tables (1D) ---------------------------------------
