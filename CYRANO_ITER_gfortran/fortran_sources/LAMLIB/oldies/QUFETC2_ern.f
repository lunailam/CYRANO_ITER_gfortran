      subroutine qufetc2_ern(onebl, iblock, toread, clean, ini, icurspe)
c	vectorized version of qufetc2.f
...
c     ;, donewr
      ...
      cabs1(zdum) = dabs(dreal(zdum)) + dabs(dimag(zdum))
        ...
        if(ini)then
cE        call zset(nthoma, czero, xsbmax, 1)
cE        call zset(nthoma, czero, xsax, 1)
cE        call zset(nthoma, czero, xsb, 1)
cE        call zset(maxnbl*nspec, czero, xsax_sp, 1)
		xsbmax(1:nthoma) = czero
	    xsax(1:nthoma)   = czero
	    xsb(1:nthoma)    = czero
	    xsax_sp(1:nthoma,1:nspec) = czero
...
cE        call zset(maxbll, czero, residu, 1)
		residu(1:maxbll) = czero
...
cE      call zset(lblt, czero, solvec, 1)
cE      call zset(lblt, czero, source, 1)
	  solvec(1:lblt) = czero
	  source(1:lblt) = czero	  		
      ish = 0
...
c     'Raw' input impedance matrix:
        do ia = 1, nrhs
         do ja = 1, nrhs
cE          do i = 1, lbl(ib)
cE          rawzij(ia,ja) = rawzij(ia,ja) + dconjg(ctom(i,ia)) * bel(i+ish,ja)
		rawzij(ia,ja) = rawzij(ia,ja) + sum (
     ;                    dconjg(ctom(1:lbl(ib),ia))*bel(1+ish:lbl(ib)+ish,ja) )
cE          end do
         end do
        end do
c     Current excitation case: store solution in solvec:
	  ...
	  if(samant)js = 1
cE        do i = 1, lbl(ib)
cE        solvec(i+ish) = solvec(i+ish) + tofacv(j) * ctom(i,js)
	    solvec(1+ish:lbl(ib)+ish) = solvec(1+ish:lbl(ib)+ish) + 
     ;                                tofacv(j) * ctom(1:lbl(ib),js)
cE        end do
        end do
      ish = lbl(1)
      end do
...
cPL        do j = 2, nrhs
cE          do i = 1, lblt
cE          source(i) = source(i) + tofacv(j) * bel(i,js)
	      source(1:lblt) = source(1:lblt) + tofacv(j) * bel(1:lblt,js)
cPL        bel(i,1) = bel(i,1) + tofacv(j) * bel(i,j)
cE          end do
        end do
c     Initialize 2nd column of BEL to zero (used in elt quadratic form calc.):
cE      call zset(lblt, czero, bel(1,2), 1)
		bel(1:lblt,2) = czero

c     Work of sources on solution, for each element: transconj(x).b
cE        do k = 1, lblt
cE        xsb(iel) = xsb(iel) + dconjg(solvec(k)) * source(k)
	    xsb(iel) = sum ( dconjg(solvec(1:lblt)) * source(1:lblt) )
cEcPL080404        xsb(iel) = xsb(iel) + dconjg(solvec(k)) * bel(k,1)
cE        end do
...
c     Residual b-Ax for each element is computed in source:
        do k = 1, lblt
cE          do i = 1, lblt
CEcPL          bel(i,1) = bel(i,1) - ael(i,k) * solvec(k)
cE          source(i) = source(i) - ael(i,k) * solvec(k)
		  source(1:lblt) = source(1:lblt) - ael(1:lblt,k) * solvec(k)
cE2	      source(1:lblt) = - matmul( ael(1:lblt,1:lblt),solvec(1:lblt) )  ! slightly slower
cE          end do
        end do

c     Quadratic form transconj(x).(b-Ax) for each element:
cE        do k = 1, lblt
CEcPL        xsbmax(iel) = xsbmax(iel) + dconjg(solvec(k)) * bel(k,1)
cE        xsbmax(iel) = xsbmax(iel) + dconjg(solvec(k)) * source(k)
		xsbmax(iel) = sum( dconjg(solvec(1:lblt)) * source(1:lblt) )
cE        end do
c     Product A.x for present element is computed in 2nd col. of block BEL:
        do k = 1, lblt
cE          do i = 1, lblt
cE          bel(i,2) = bel(i,2) + ael(i,k) * solvec(k)
		  bel(1:lblt,2) = bel(1:lblt,2) + ael(1:lblt,k) * solvec(k)
cE2		  bel(1:lblt,2) = matmul( ael(1:lblt,1:lblt),solvec(1:lblt) ) ! slightly slower
cE          end do
        end do
c     Quadratic form transconj(x).A.x for present element:
cE        do k = 1, lblt
cE        xsax(iel) = xsax(iel) + dconjg(solvec(k)) * bel(k,2)
		xsax(iel) = sum ( dconjg(solvec(1:lblt)) * bel(1:lblt,2) )
cE        end do


	    if(recalc_by_species_now)then
cE            do k = 1, lblt
cE            xsax_sp(iel,icurspe) = xsax_sp(iel,icurspe)
cE     ;                           + dconjg(solvec(k)) * bel(k,2)
            xsax_sp(iel,icurspe) = sum ( dconjg(solvec(1:lblt))*bel(1:lblt,2) )
...
cPL      residu(i) = bel(i+lbl(1),1)
      ...
      soupow = soupow + xsb(iel)
        ...
        if(clean)then
	      ael(1:lblt,1:lblt) = czero
	      bel(1:lblt,1:nrhs) = czero
        end if
...
cE		rawzij(1:nrhs,1:nrhs) = rawzij(1:nrhs,1:nrhs) * dcmplx(0.d0,2.d0)
          end do
        end do
      write(6,*)' '
...
cE          matpow = 0.5 * sum ( matmul( zij_quf(1:ntoant,1:ntoant),
cE     ;                         dconjg(tancur(1:ntoant))*tancur(1:ntoant) ) )
