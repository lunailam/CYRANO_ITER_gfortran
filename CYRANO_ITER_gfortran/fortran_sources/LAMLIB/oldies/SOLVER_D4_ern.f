...
cERN	    call zaxpy (lbl2, cone, ctom(1,j+lbl2), 1, bel(1,j), 1)
		bel(1:lbl2,1:nrhs) = bel(1:lbl2,1:nrhs)+ctom(1:lbl2,1+lbl2:nrhs+lbl2)
cPL          do i = 1, lbl2
cPL          bel(i,j) = bel(i,j) + ctom(i,j+lbl2)
cPL          end do
cERN        end do
...
cERN	call zcopy(lbl2, ael(1,j2), 1, ctom(1,j), 1)
	 ctom(1:lbl2,j) = ael(1:lbl2,j2)
c      do 18 i = 1, lbl2
c      ctom(i,j) = ael(i,j2)
  18  continue
      end if
...
cERN      do 19 j = 1, nrhs
cERN      j2 = j + lbl3
cERN	call zcopy(lbl2, bel(1,j), 1, ctom(1,j2), 1)
	 ctom(1:lbl2,1+lbl3:nrhs+lbl3) = bel(1:lbl2,1:nrhs)
c      do 19 i = 1, lbl2
c      ctom(i,j2) = bel(i,j)
cERN  19  continue
      end if
...
cERN          do 20 j = 1, lbl2
cERN          call zcopy(lbl3, ael(lbl2+1,j), 1, btom(1,j), 1)
		    btom(1:lbl3,1:lbl2) = ael( 1+lbl2:lbl3+lbl2 , 1:lbl2 )
c          do 20 i = 1, lbl3
...
cPL          do j = 1, lbl3
cPL            do i = 1, lbl3
cPL            ctom(i,j) = ael(i+lbl2,j+lbl2)
cPL            end do
cPL          end do
...
cPL            do i = 1, lbl3
cPL            ctom(i,j+lbl3) = bel(i+lbl2,j)
cPL            end do
cERN          end do
        end if
      end if
...
c
cERN	call dccgcg (lbl3, ael(lbl2+1,lbl2+1), ldael, ael, ldael)
	 ael(1:lbl3,1:lbl3) = ael(1+lbl2:lbl3+lbl2 , 1+lbl2:lbl3+lbl2)
c      do 21 j = 1, lbl3
c      j2 = j + lbl2
c      do 21 i = 1, lbl3
...
c  21  continue
cERN      do j = 1, nrhs
cERN	call zcopy(lbl3, bel(lbl2+1,j), 1, bel(1,j), 1)
	 bel(1:lbl3,1:nrhs) = bel(1+lbl2:lbl3+lbl2,1:nrhs)
c      do 23 i = 1, lbl3
c      bel(i,j) = bel(i+lbl2,j)
cERN  23  continue
cERN	call zset(lbl2, czero, bel(lbl3+1,j), 1)
		bel(lbl3+1:lbl3+lbl2 , 1:nrhs) = czero
...
cERN      do 25 j = 1, lbl2
cERN	call zset(ldael, czero, ael(1,lbl3+j), 1)
