...
c
     ...
     ;, jnor(npfft+1), jnorav, r0oravi, gfac, gfacn, trata(npfft+1), rtr

      complex*16 cdeid

      external inscr, gauss, totcun, g, circ_shear, idamin, idamax, i1st, nsum, gfac, gfacn, cdeid
...
C       Magnetic field angle sine, cosine and radial (y) derivative: (1D table for circular cross-section)
	  ...
	  si1tab(i) = si1
	  co1tab(i) = co1
	  eqt(i,1:npp,14) = cotab(i)		  ! cos(THETA)
	  eqt(i,1:npp,15) = sitab(i)		  ! sin(THETA)
	  eqt(i,1:npp,16) = si1tab(i)	* rnori ! d sin(THETA)/drho
	  eqt(i,1:npp,17) = 0.d0		  ! d sin(THETA)/dtheta
	  end if
c
...
c	      !!!Need to decide the link between geneq and READ_FLUX!!!
		...
		g12n = 0.d0
		ntn = 1.d0
            bmotab(i,ipo) = bmodul ! In circular, bmodul is produced by comag1
...
c         ------------
          qtab(i) = q
	    eqta1d(i,4) = q
	    eqta1d(i,5) = gfacn(rho)
	    eqta1d(i,9) = twopi**2 * rho
	    if(.not.cyl) eqta1d(i,9) = eqta1d(i,9) * ra
          eqta1d(i,10) = twopi * rho
            if(ireg.eq.1)then
            eqta1d(i,8) = dmax1(1.d0-(rho*api)**2, 0.d0)**alpha
            else
            eqta1d(i,8) = 0.d0
            end if
...
c	    geometry, such as the pitch angle, toroidal magnetic field, Psi, etc...) 
        ...
        if(circ)then
c       ------------
        eqta1d(i,3) = totcun(rho)
        eqta1d(i,6) = mu0 * eqta1d(i,3) / gfacn(rho)
c        eqta1d(i,7) = still using numerical radial integration for psi(rho)
...
cPL8/2/05 doesn't handle the circular case
		  if(geneq)then
...
c      else
c     Circular: qtab and eqta1d(i,4) already filled; following was filled above:
c        do i = 1, nabsci
c	  eqt(i,1:npp,13) = tgth		  ! tan(THETA)
c	  eqt(i,1:npp,14) = cotab(i)		  ! cos(THETA)
c	  eqt(i,1:npp,15) = sitab(i)		  ! sin(THETA)
c	  eqt(i,1:npp,12) = co*tgthor         ! sin(THETA)/rho
c	  bmotab(i,ipo) = bmodul	  ! |B| = sqrt(Btor^2 + Bpol^2)      done after call comag1 above in circular
      end if     
...
cPL3/2/05: if cokpco variable per region, could make some savings tabulating only in the cokcpo regions
c	cokpco = copcov(ireg)
...
cERN
cERN	    do i = 1, nabsci
cERNc     11.1) dThbar/dTheta and 'H' coefficient: 
cERN          ckt(i,1:npp,1) = 1.d0
cERNc	    Makes hachi=sin(THETA)/rho the inverse of cursive H in paper:
cERN	    hachi(i) = eqt(i,1,12)
cERN
cERNc	     jnor(1:npp) = 1.d0 / eqt(i,1:npp,1)        ! Jn / R = here 1/R; 
cERNc          jnorav = 2.d0 * sum(jnor(2:npp-1)) + jnor(1) + jnor(npp) 
cERNc          jnorav = jnorav / dfloat(npfft) 
cERN          r0oravi = dsqrt(1.d0 - (abscis(i)*r0i)**2) ! 1/(poloidal average of R0/R) = 1/bfou(0)
cERNc     11.2) dPhibar/dTheta: 
cERNc		ckt(i,1:npp,2) = qtab(i) * (1.d0 - jnor(1:npp) / jnorav)      ! q * (1 - (1/R)/<1/R>)
cERN	    ckt(i,1:npp,2) = qtab(i) * (1.d0 - r0oravi * r0orta(i,1:npp)) ! q * (1 - (R0/R)/<R0/R>)
cERN
cERNc     11.3) Thbar and Phibar, using analytical expressions: 
cERN	    ckt(i,1:npp,3) = polang(1:npp)
cERN	    rho = abscis(i)
cERN	    rtr = dsqrt((r0-rho)/(r0+rho))
cERN	    ckt(i,1:npp,4) = qtab(i) * (polang(1:npp) - 2.d0 * datan(rtr * dtan(0.5d0*polang(1:npp))))
cERNc	    ckt(i,npp,4) = 0.d0

cERNc     11.4) dThbar/drho: identically zero in circular; dPhibar/drho using analytical expression: 
cERN          ckt(i,1:npp,6) = 0.d0
cERN	    ckt(i,1:npp,7) = circ_shear(abscis(i)) * ckt(i,1:npp,4) 
cERN     ;                     + (qtab(i)/r0oravi) * dsin(polang(1:npp)) / eqt(i,1:npp,1)
cERNc     ;                  + (qtab(i)/dsqrt(1-(abscis(i)*r0i)**2)) * dsin(polang(1:npp)) / eqt(i,1:npp,1)
cERN         end do  ! Radial loop

cERNcERN 27Jan2005 (2nd trial)
cERNc     11.5) 2D table of exp[+i.n.(Phibar-Phi)] in thetabar mesh (same as theta mesh in circular)
cERN	    do i = 1, nabsci	! radial loop
cERNc	    Compute exp[+i.n.(Phibar-Phi)]		  	
cERN	    einphb(i,1:npp) = dcmplx(dcos(n*ckt(i,1:npp,4)), dsin(n*ckt(i,1:npp,4)))
cERNc	or  einphb(i,1:npp) = cdeid_vec(n*ckt(i,1:npp,4))
cERN	    end do

cERN	  else ! Non-circular case
c       ====
...
ccccccccccccERN: DISABLED 15 Feb 2005 cccccccccccccccc
