	...
	USE DFPORT		! ccccccccc ERNESTO cccccccccccccc
...
c     Assemble curl.curl and displacement current contribution:
	  ...
	  print *, '    ( element #',iel,' )' 
	  print *, '    ( node radius = ', abscis(intab),')'
	  print *
	  tempo0 = etime (T1)
      if(gensys .or. .not.onlyab_now)then
	  print *, '    --> (GENERA3.f) Begin ASCURL:', tempo0, 'sec' 
c	  print *, kappa
	call ascurl
	  tempo  = etime (T1)
        print *, '    --> (GENERA3.f)   End ASCURL:', tempo, 'sec' 
	  print *, '         Time used:', tempo-tempo0, 'sec'
	  print *
	end if
...
c     Assemble plasma contribution:
      ...
      if(.not. vacuum(ireg))then  ! Only process plasma regions
 	  ...
 	  tempo0 = etime (T1)
	  print *, '    --> (GENERA3.f) Begin ASPLAS + QUFETC2' // 
     ;           ' with species loop:', tempo0, 'sec'
          do isp = 1, nspec
...
c         get plasma active and if required reactive power for each species and clean up ael, bel
 	  ...
 	  tempo = etime (T1)
        print *, '    --> (GENERA3.f)   End ASPLAS + QUFETC2' // 
     ;           ' with species loop:', tempo, 'sec'
	  print *, '         Time used:', tempo-tempo0, 'sec'
	  print *

        else  ! Usual treatment:
	  tempo0 = etime (T1)
	  print *, '    --> (GENERA3.f) Begin ASPLAS:', tempo0, 'sec' 
	  call asplas(.false., idum)
	  tempo  = etime (T1)
        print *, '    --> (GENERA3.f)   End ASPLAS:', tempo, 'sec' 
	  print *, '         Time used:', tempo-tempo0, 'sec'
	  print *
        end if
	end if
...
ccccccccccccccccccccccccccc ERNESTO cccccccccccccccccccccccc
  	  tempo0 = etime (T1)
	  print *, '    --> (GENERA3.f) Begin FBSTEP:', tempo0, 'sec' 
      call fbstep(isca, scaf(1), scaf(1))
	  tempo  = etime (T1)
        print *, '    --> (GENERA3.f)   End FBSTEP:', tempo, 'sec' 
	  print *, '         Time used:', tempo-tempo0, 'sec'
	  print *
...
ccccccccccccccccccccccccccc ERNESTO cccccccccccccccccccccccc
	  ...
	  tempo0 = etime (T1)
	  print *, '    --> (GENERA3.f) Begin REGJUM:', tempo0, 'sec' 
	call regjum(ireg,'L')
	  tempo  = etime (T1)
        print *, '    --> (GENERA3.f)   End REGJUM:', tempo, 'sec' 
	  print *, '         Time used:', tempo-tempo0, 'sec'
	  print *
	end if
cccccccccccccccccccccccccccccccccccccccccccccccccccccccc 
