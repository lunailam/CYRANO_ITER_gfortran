...
cPL-ERN25Nov04 flip sign
      v(3,5) = dcmplx(dfou(1) * kprn, 0.d0)
c      v(3,5) = dcmplx(- dfou(1) * kprn, 0.d0)
      if(k.lt.0)v(3,5) = - v(3,5)
      v(5,3) = - v(3,5)
c      v(5,3) = v(3,5)
      ...
      v(3,3) = v(1,1)
cPL-ERN25Nov04 flip sign
      v(3,5) = dcmplx(dfou(k2)*kprn, 0.d0)
...
cc	else
cc	v(3,5) = v(3,5) + n * kprn * acokfou(k)		 ! ~ FFT(  dPhibar/dtheta)
cc	v(5,3) = v(5,3) + n * kprn * acokfou(k)		 ! ~ FFT(  dPhibar/dtheta)
cc	end if

	v(5,5) = v(5,5) + n*n*yinv * ccokfou(k)		 ! ~ FFT(R/Ro.(dPhibar/dtheta)^2) - even, real
c     ;	            + (2*m+k)*n*yinv*bcokfou(k)
...
c	else
c	v(1,5) = v(1,5) + n * kprn * y * dcokfou(k2)	 ! ~ FFT(a.dPhibar/drho) 
c	v(5,1) = v(5,1) + n * kprn * y * dcokfou(k2)	 ! ~ FFT(a.dPhibar/drho) 
c	end if
...
c	else
c	v(5,6) = v(5,6) + ci * n * y * ecokfou(k2)	 ! ~ FFT(a.R/Ro.dPhibar/drho) 
c	v(6,5) = v(6,5) - ci * n * y * ecokfou(k2)	 ! ~ FFT(a.R/Ro.dPhibar/drho) 
c	end if
...
c	3) Toroidal contributions:
	v(1,1) = v(1,1) + n*n*yinv * ccokfou(k2)		 ! ~ FFT(R/Ro.(dPhibar/dtheta)^2)  - even, real
c     ;	            + (2*m+k)*n*yinv*bcokfou(k2)

	v(3,3) = v(3,3) + n*n * y * fcokfou(k2)		 ! ~ FFT(a2.R/Ro.(dPhibar/drho)^2) - even, real
...
c	else
c	v(1,3) = v(1,3) + ci * n * yinv * bcokfou(k2) ! ~ FFT(R. dPhibar/dtheta))
c     ;                + n * m * ecokfou(k2)		 ! ~ FFT(a.R/Ro. dPhibar/drho))
c     ;				+ n * n * gcokfou(k2)		 ! ~ FFT(a.R/Ro.dPhibar/dtheta.dPhibar/drho))

c	v(3,1) = v(3,1) - ci * n * yinv * bcokfou(k2) ! ~ FFT(R. dPhibar/dtheta))
c     ;                + n * mpk * ecokfou(k2)		 ! ~ FFT(a.R/Ro. dPhibar/drho))
c     ;				+ n * n * gcokfou(k2)		 ! ~ FFT(a.R/Ro.dPhibar/dtheta.dPhibar/drho))
...
c	else
c      v(3,4) = v(3,4) + ci * n * y * ecokfou(k2)	 ! ~ FFT(a.R/Ro.dPhibar/drho)
c      v(4,3) = v(4,3) - ci * n * y * ecokfou(k2)	 ! ~ FFT(a.R/Ro.dPhibar/drho)
c	end if
