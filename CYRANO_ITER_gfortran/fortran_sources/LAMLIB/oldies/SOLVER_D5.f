...
cERN        do j = 1, lbl2
cERN	    call zaxpy (lbl2, cone, ctom(1,j), 1, ael(1,j), 1)
cERNcPL          do i = 1, lbl2
cERNcPL          ael(i,j) = ael(i,j) + ctom(i,j)
cERNcPL          end do
cERN        end do
		  ael(1:lbl2,1:lbl2) = ael(1:lbl2,1:lbl2) + ctom(1:lbl2,1:lbl2)

cERN        do j = 1, nrhs
cERN	    call zaxpy (lbl2, cone, ctom(1,j+lbl2), 1, bel(1,j), 1)
cERNcPL          do i = 1, lbl2
cERNcPL          bel(i,j) = bel(i,j) + ctom(i,j+lbl2)
cERNcPL          end do
cERN        end do
		  bel(1:lbl2,1:nrhs) = bel(1:lbl2,1:nrhs) + 
     ;						   ctom(1:lbl2,1+lbl2:nrhs+lbl2)
...
c
          if(modele .eq. 0)then
cERN	call zgemm ('N', 'N', lbl2, lbl2, lbl1, -cone, btom, ldbt
cERN     ;          , ctom, ldct, cone, ael, ldael)
		ael(1:lbl2,1:lbl2) = ael(1:lbl2,1:lbl2) - 
     ;               matmul( btom(1:lbl2,1:lbl1), ctom(1:lbl2,1:lbl1) )
cERNcPL      do 13 j = 1, lbl2
cERNcPL      do 13 k = 1, lbl1
cERNcPL      do 13 i = 1, lbl2
cERNcPL      ael(i,j) = ael(i,j) - btom(i,k) * ctom(k,j)
cERNcPL  13  continue

cERN	call zgemm ('N', 'N', lbl2, nrhs, lbl1, -cone, btom, ldbt
cERN     ;          , ctom(1,lbl2+1), ldct, cone, bel, ldbel)
		bel(1:lbl2,1:nrhs) = bel(1:lbl2,1:nrhs) - 
     ;        matmul( btom(1:lbl2,1:lbl1), ctom(1+lbl2:nrhs+lbl2,1:lbl1) )
cPL      do 14 j = 1, nrhs
cPL      do 14 k = 1, lbl1
cPL      do 14 i = 1, lbl2
cPL      bel(i,j) = bel(i,j) - btom(i,k) * ctom(k,j+lbl2)
cPL  14  continue
          else

cERN	call zgemm ('N', 'N', lbl2, lbl2, lbl1, -cone, ael, ldael
cERN     ;          , ctom, ldct, cone, ael(1,lbl1+1), ldael)
		ael(1:lbl2,1+lbl1:lbl2+lbl1) = ael(1:lbl2,1+lbl1:lbl2+lbl1) - 
     ;               matmul( ael(1:lbl2,1:lbl1), ctom(1:lbl2,1:lbl1) )

cERNcPL      do 15 j = 1, lbl2
cERNcPL      do 15 k = 1, lbl1
cERNcPL      do 15 i = 1, lbl2
cERNcPL      ael(i,j+lbl1) = ael(i,j+lbl1) - ael(i,k) * ctom(k,j)
cERNcPL  15  continue

cERN	call zgemm ('N', 'N', lbl2, nrhs, lbl1, -cone, ael, ldael
cERN     ;          , ctom(1,lbl2+1), ldct, cone, bel, ldbel)
		bel(1:lbl2,1:nrhs) = bel(1:lbl2,1:nrhs) - 
     ;        matmul( ael(1:lbl2,1:lbl1), ctom(1+lbl2:nrhs+lbl2,1:lbl1) )

cERNcPL      do 16 j = 1, nrhs
cERNcPL      do 16 k = 1, lbl1
cERNcPL      do 16 i = 1, lbl2
cERNcPL      bel(i,j) = bel(i,j) - ael(i,k) * ctom(k,j+lbl2)
cERNcPL  16  continue
          end if
      end if
...
cPL 23/5/04 dccgcg has error when first argument is zero:
        if(lbl3.gt.0)then
cERN	  call dccgcg (lbl3, ael(lbl2+1,lbl2+1), ldael, ctom, ldct)
		 ctom(1:lbl3,1:lbl3) = ael(1+lbl2:lbl3+lbl2,1+lbl2:lbl3+lbl2)
cERNcPL          do j = 1, lbl3
cERNcPL            do i = 1, lbl3
cERNcPL            ctom(i,j) = ael(i+lbl2,j+lbl2)
cERNcPL            end do
cERNcPL          end do

		 ctom(1:lbl3,1+lbl3:nrhs+lbl3) = bel(1+lbl2:lbl3+lbl2,1:nrhs)
cERN          do j = 1, nrhs 
cERN	    call zcopy(lbl3, bel(lbl2+1,j), 1, ctom(1,lbl3+j), 1)	
cERNcPL            do i = 1, lbl3
cERNcPL            ctom(i,j+lbl3) = bel(i+lbl2,j)
cERNcPL            end do
cERN          end do
        end if
      end if
...
cERN	vec(ivec:nro) = a(1:nro,j)
	ivec = ivec + nro
CERNcPL      do 1 i = 1, nro
cERNcPL      ivec = ivec + 1
cERNcPL      vec(ivec) = a(i,j)
   1  continue
...
cERN	a(1:nro,j) = vec(ivec:nr)
	ivec = ivec - nro
cERNcPL      do 2 i = nro, 1, -1
cERNcPL      a(i,j) = vec(ivec)
cERNcPL      ivec = ivec - 1
   2  continue
      end if
