...
c      of the solution.)
      ...
      integer 
     ;  i, im, is, iexc, i1, j, ja, jant, jm, k1, k2, km, kp, mml, mmk
...
c          store(1:npfft+1) = ckt(intab,1:npfft+1,4) - phibarac(j)
          store(1:npp) = ckt(intab,1:npp,4) - phibarac(j)
            do i = 1, npp
            foutra(i-1,1) = dconjg(einphb(intab,i)) * einpbac * dnfi
            end do
            if(updsym)then
c           Use up-down symmetry:
              do i = 1, npp-1
              foutra(npp+i-1,1) = dconjg(foutra(npp-i-1,1))
              end do
            end if
...
c         Estimate poloidal spectrum range for exp[-in*(phibar-phi)]: 
          epsa = rhoant * r0i
...
c         Phibar at antenna strap centre:
          call interp1(polang, polang(npfft+1), ckt(intab,1,4), nabplo, npfft+1, theaa(j), phibarac(j), 1, 'R')
...
c          rcura(i,j) = torfac * foutra(m,1)
          polfac = czero
            do k = -npft2, npft2
            mmk = m - k
            polfac = polfac + foutra(k,1) * cdeid(- mmk * thba(j)) * dtheab(j) * pi2i * dsinc(mmk * dtheab(j) * 0.5d0)
            end do
          rcura(i,j) = torfac * polfac

c         Get current density spectrum:
          polfac = czero
          polcha = czero
          k1 = max0(-ncrot, moant(1)-m )
          k2 = min0( ncrot, moant(nmoant)-m)
            do k = k1, k2
            mpk = m + k
            polfac = polfac + foutra(mpk,1) * bfou(iabs(k))
            end do
c         Resulting polfac if Fourier transform of R0/R times poloidal Morse pulse
          cura(i,j) = cura(i,j) + torfac * polfac * r0i
          if(falen(j))chaa(i,j) = chaa(i,j) + torfac * polcha * r0i * (deltaa(j)/(ci * omegag * rhoant))
	    end if

c          else  ! Noncircular case, both std and constant k//; and circular with const. k//
c19/1/05:
