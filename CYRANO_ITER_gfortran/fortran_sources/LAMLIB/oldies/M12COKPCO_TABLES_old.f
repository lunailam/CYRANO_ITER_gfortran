...
c	Variables
	real*8  :: Avec(2000)
	integer :: OpenStat
      integer :: i, j, mdiffvec(161), Nmdiff, NAvec
	logical :: resp
...
c     1) Poloidal mode couplings (mdiff = m_i - m_j)
	...
	Nmdiff = 2*klim + 1   ! Nmdiff = klim + 1
	do j = 1, Nmdiff
		mdiffvec(j) = (j-1) - klim  ! mdiffvec(j) = (j-1)
	end do
...
c     3) Create file with index of radial elements
	...
	open (UNIT = 2222, FILE = '../../M12tables/index_radii.dat', 
     ;      STATUS = "REPLACE", IOSTAT = OpenStat, ACTION = "WRITE")
	      if (OpenStat > 0) then
	         print *,'Error writing file: ../../M12tables/index_rad.dat'
	      end if
	

c     4) First radial element (magnetic axis) + + + + + + + + + + + + + + + + + + +
	
	intab = 2  ! cERN WARNING: Why not intab=1 ???
	print *, '               ===================== '				
...
c            - if ALLRAD_TABLES = .F. -> consider only element nodes 
	...
	if (.not. ALLRAD_TABLES) then ! Considering only nodes -> ifiabs(iel)

          do iel = 2, nele 	! Loop over nodes						
...
c            5.2) Calculate the M12 matrix for several A and cosXo values.	       
	...
	else  ! ALLRAD_TABLES = .TRUE. -> Considering all elements

          do iel = 3, 2500  ! Loop over all elements: cERN WARNING: subst.2500						

		   intab = iel													
		   if( vacuum(iregoa(iel)) ) goto 3333
