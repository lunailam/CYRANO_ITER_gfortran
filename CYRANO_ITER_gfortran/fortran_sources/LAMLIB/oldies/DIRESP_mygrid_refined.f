      SUBROUTINE DIRESP_mygrid_refined(isp)
...
cERN	k//-loop is now the most external one !!!!!!!!!!!!!!!!!!!
...
c     ``````
cEnow      end do	! k// loop (im = 1 : 2*nmoant-1)
cEnowC     >>>>>>
