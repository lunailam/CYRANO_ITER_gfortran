...
c     ;, thba, theab1, theab2, dtheab
     ...
     ;, vec(6), tra, trac, trho, tthe, tpl, tmi, tpa, fapl, fami, fapa
     ;, cdeid
c     ;, t
...
c         Right part of last element in region
c         = left part of current (boundary) block:
          ideca = (mr - 1) * icolo
          write(nofile,1000)moant(i), cura(i,ja), rcura(i,ja)
     ;  , rcurat(i,ja), chaa(i,ja) / eps0
...
c       isc is faraday shield index
      ...
      if(.not.atabou .and. ja.le.neffan .and. feeder(ja) .and.
     ;   iel.ge.ifiel(irea1))then
c     Feeders volume contribution:
c     n.b. when FALEN is true, remember the different current normaliz. 
c     according to antenna end type.
      irea = ireg
      isuba = isubr
      iela = iel
...
c don't forget an exp(i*n*(phibar(theta)-phibarac(ja))), interpolate phibar etc
          if(circ)then
c         ~~~~~~~~~~~~
          do im = 1, nmoant
          m = moant(im)
          ...
          inl = nmode(iela-1).gt.0 .and.
     ;          m.ge.minf(iela-1) .and. m.le.msup(iela-1)
          inr = nmode(iela).gt.0 .and.
     ;          m.ge.minf(iela) .and. m.le.msup(iela)
...
c          intab = ireg + (ngauss + 1) * (iel - 1)
          intab = ifiabs(iel)
            ...
            intab = intab + 1
c           Jn at inlet:
            call interp1(polang, polang(npfft+1), eqt(intab,1,9), nabplo
     ;    , npfft+1, thea1(ja), jaint1(ig), 1, 'R')
c     ;    , npfft+1, theab1, jaint1(ig), 1, 'R')
c           Ntn at inlet:
            call interp1(polang, polang(npfft+1), eqt(intab,1,7), nabplo
     ;    , npfft+1, thea1(ja), ntint1(ig), 1, 'R')
c     ;    , npfft+1, theab1, ntint1(ig), 1, 'R')
c           Gn at inlet:
            call interp1(polang, polang(npfft+1), eqt(intab,1,8), nabplo
     ;    , npfft+1, thea1(ja), gint1(ig), 1, 'R')
c           cos Theta at inlet:
            call interp1(polang, polang(npfft+1), eqt(intab,1,14), nabplo
     ;    , npfft+1, thea1(ja), coi1(ig), 1, 'R')
c           sin Theta at inlet:
            call interp1(polang, polang(npfft+1), eqt(intab,1,15), nabplo
     ;    , npfft+1, thea1(ja), sii1(ig), 1, 'R')
...
c             Jn at outlet:
              call interp1(polang, polang(npfft+1), eqt(intab,1,9), nabplo
     ;      , npfft+1, thea2(ja), jaint2(ig), 1, 'L')
c     ;      , npfft+1, theab2, jaint2(ig), 1, 'L')
c             Ntn at outlet:
              call interp1(polang, polang(npfft+1), eqt(intab,1,7), nabplo
     ;      , npfft+1, thea2(ja), ntint2(ig), 1, 'L')
c     ;      , npfft+1, theab2, ntint2(ig), 1, 'L')
c             Gn at outlet:
              call interp1(polang, polang(npfft+1), eqt(intab,1,8), nabplo
     ;      , npfft+1, thea2(ja), gint2(ig), 1, 'L')
c             cos Theta at outlet:
              call interp1(polang, polang(npfft+1), eqt(intab,1,14), nabplo
     ;      , npfft+1, thea2(ja), coi2(ig), 1, 'R')
c             sin Theta at outlet:
              call interp1(polang, polang(npfft+1), eqt(intab,1,15), nabplo
     ;      , npfft+1, thea2(ja), sii2(ig), 1, 'R')
     ...
     ;          m.ge.minf(iela) .and. m.le.msup(iela)
          ...
          trac = dconjg(tra)
           ...
           if(falen(ja))then
           write(nofile,*)'Propagation effects along antenna not implemented yet
     ; in general geometry, are ignored'
           end if
...
c             M23 basis function type is 2 for radial and 1 for poloidal component!
                ...
                bel(ideca+3,ja) = bel(ideca+3,ja) + fat * bafgn(2,0,1,ig)
     ;          * rle
	          end if
              bel(idecam+1,ja) = bel(idecam+1,ja) + far * bafgn(2,0,2,ig)
                ...
                bel(ideca2+3,ja) = bel(ideca2+3,ja) + fat * bafgn(4,0,1,ig)
     ;          * rle
                end if
              else if(styp(isuba).eq.'HEC')then	        
	        tpl =  wga(ig) * sqrt2i * 
     ;        (trac * (jon2(ig) + ci * coi2(ig) * gon2(ig)) 
     ;        - tra * (jon1(ig) + ci * coi1(ig) * gon1(ig)))
	        tmi =  wga(ig) * sqrt2i * 
     ;        (trac * (jon2(ig) - ci * coi2(ig) * gon2(ig)) 
     ;        - tra * (jon1(ig) - ci * coi1(ig) * gon1(ig)))
	        tpa =  wga(ig) *  
     ;        (trac * sii2(ig) * gon2(ig) - tra * sii1(ig) * gon1(ig))
              fapl = facv * tpl
              fami = facv * tmi
              fapa = facv * tpa
                if(inl)then
                bel(ideca+1,ja) = bel(ideca+1,ja) + fapl * bafgn(1,0,1,ig)
                bel(ideca+2,ja) = bel(ideca+2,ja) + fapl * bafgn(2,0,1,ig)
     ;          * rle
                bel(ideca+3,ja) = bel(ideca+3,ja) + fami * bafgn(1,0,1,ig)
                bel(ideca+4,ja) = bel(ideca+4,ja) + fami * bafgn(2,0,1,ig)
     ;          * rle
                bel(ideca+5,ja) = bel(ideca+5,ja) + fapa * bafgn(1,0,1,ig)
                bel(ideca+6,ja) = bel(ideca+6,ja) + fapa * bafgn(2,0,1,ig)
     ;          * rle
                end if
                if(inr)then
                bel(ideca2+1,ja) = bel(ideca2+1,ja) + fapl * bafgn(3,0,1,ig)
                bel(ideca2+2,ja) = bel(ideca2+2,ja) + fapl * bafgn(4,0,1,ig)
     ;          * rle
                bel(ideca2+3,ja) = bel(ideca2+3,ja) + fami * bafgn(3,0,1,ig)
                bel(ideca2+4,ja) = bel(ideca2+4,ja) + fami * bafgn(4,0,1,ig)
     ;          * rle
                bel(ideca2+5,ja) = bel(ideca2+5,ja) + fapa * bafgn(3,0,1,ig)
                bel(ideca2+6,ja) = bel(ideca2+6,ja) + fapa * bafgn(4,0,1,ig)
     ;          * rle
                end if
              end if
            end do
