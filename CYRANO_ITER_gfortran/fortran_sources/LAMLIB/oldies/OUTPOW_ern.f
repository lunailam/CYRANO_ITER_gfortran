...
c             this is only correct if outgau=.t.! otherwise 2d interpol.
              call interp1(polang, polang(npfft+1), bmotab(intab,1), nabplo,
     ;        npfft+1, polplo, bmlvec, iplom1, 'R')
              call interp1(polang, polang(npfft+1), r0orta(intab,1), nabplo,
     ;        npfft+1, polplo, r0rvec, iplom1, 'R')
                if(.not.circ)then
                call interp1(polang, polang(npfft+1), eqt(intab,1,14), nabplo,
     ;          npfft+1, polplo, cov, iplom1, 'R')
                call interp1(polang, polang(npfft+1), eqt(intab,1,15), nabplo,
     ;          npfft+1, polplo, siv, iplom1, 'R')
                call interp1(polang, polang(npfft+1), eqt(intab,1,7), nabplo,
     ;          npfft+1, polplo, ntnt, iplom1, 'R')
                end if
              else
c             Use up-down symmetry:
c             this is only correct if outgau=.t.! otherwise use 2d interpol.
              ian = nploth / 2 + mod(nploth,2)
              nan = nploth - ian + 1
              call interp1(polang, polang(npfft/2+1), bmotab(intab,1), nabplo,
     ;        npfft/2+1, polplo, bmlvec, ian, 'R')
              call dcopy(ian, bmlvec, 1, bmlvec(nan), -1)
              call interp1(polang, polang(npfft/2+1), r0orta(intab,1), nabplo,
     ;        npfft/2+1, polplo, r0rvec, ian, 'R')
c             now r0rvec contains Ra over R, not the inverse!
              call dcopy(ian, r0rvec, 1, r0rvec(nan), -1)
                if(.not.circ)then
                call interp1(polang, polang(npfft/2+1), eqt(intab,1,14),
     ;          nabplo, npfft/2+1, polplo, cov, ian, 'R')
                call dcopy(ian, cov, 1, cov(nan), -1)
                call interp1(polang, polang(npfft/2+1), eqt(intab,1,15),
     ;          nabplo, npfft/2+1, polplo, siv, ian, 'R')
                call dcopy(ian, siv, 1, siv(nan), -1)
                call interp1(polang, polang(npfft/2+1), eqt(intab,1,7),
     ;          nabplo, npfft/2+1, polplo, ntnt, ian, 'R')
                call dcopy(ian, ntnt, 1, ntnt(nan), -1)
                else
                si = sitab(intab)
                co = cotab(intab)
                call dset(nploth, co, cov, 1)
                call dset(nploth, si, siv, 1)
                call dset(nploth, eqt(intab,1,7), ntnt, 1)
                end if
              end if
...
c			 Sum for total active power per element:
		  ...
		  t(1) = trapeze(istp(ireg), absout(ist), powden(ist,ispe,1), 1, 1)
		  t(2) = trapeze(istp(ireg), absout(ist), powden(ist,ispe,2), 1, 1)
		  parpow(ispe,1) = parpow(ispe,1) + t(1)
		  parpow(ispe,2) = parpow(ispe,2) + t(2)
		  tt = tt + t(ii)
	   end if
...
c	Total power
	powden(1:ist11,nspec+1,1) = powden(1:ist11,nspec+1,1)
     ;                          + powden(1:ist11,ispe,1)
	powden(1:ist11,nspec+1,2) = powden(1:ist11,nspec+1,2)
     ;                          + powden(1:ist11,ispe,2)
...
cERN	Aplly geom. factor to power densities
...
cERN        end do
	...
	powden(1:ist11,1:nspec+1,:) = powden(1:ist11,1:nspec+1,:) * fac5
	...
	open (UNIT = 701, FILE = paplda // 'powerdens1D_FLR0.dat', 
     ;      STATUS = "REPLACE", ACTION = "WRITE")
		  ...
		  write(701,*), "rho(m)  ", paname(1:nspec), "total"
            do i = 1, nabsci
		     write(701,GGs), abscis(i), powden(i,1:nspec+1,1)
		  end do
	close(701)
	...
	open (UNIT = 702, FILE = paplda // 'powerdens1D_FLR2.dat', 
     ;      STATUS = "REPLACE", ACTION = "WRITE")
		  ...
		  write(702,*), "rho(m)  ", paname(1:nspec), "total"
            do i = 1, nabsci
		     write(702,GGs), abscis(i), powden(i,1:nspec+1,2)
		  end do
	close(702)
	...
	open (UNIT = 703, FILE = paplda // 'dPoynting_dr.dat', 
     ;      STATUS = "REPLACE", ACTION = "WRITE")
		  write(703,*),"Radial derivative of inward Poynting flux (dPoynt/dr)"
            do i = 1, nabsci
		     write(703,GGs), abscis(i),  dpoydr(i)
		  end do
	close(703)
	...
	open (UNIT = 701, FILE = paplda // 'power_quotient.dat', 
     ;      STATUS = "REPLACE", ACTION = "WRITE")
		  write(701,*),"sp.absorbing in wave calc./dPoydr"
            do i = 1, nabsci
		     write(701,GGs), abscis(i), powquo(i)
		  end do
	close(701)
