...
cERN  ccccccccccccccc Opening files for cokpco OUTPUT cccccccccccccccccc
	  ...
	  if(WRITE_OUTPUT) then
	     open (UNIT = 5555, FILE = '../../M12run/index_radii.dat', 
     ;           STATUS = "REPLACE", IOSTAT = OpenStat, ACTION = "WRITE")
	       if (OpenStat > 0) then
		      print *, 'Error creating file: ../../M12run/index_radii.dat'
	          stop
	       end if	   	
	  end if

	  if(WRITE_REPORT) then
           open (UNIT = 7777, FILE = '../../cokpco_report.txt', 
     ;           STATUS = "REPLACE", ACTION = "WRITE")
	     write(7777,*)'====================================================='
		 write(7777,*)'Calculation of plasma response M12 for k// = constant'
		 write(7777,*)'====================================================='
		 write(7777,*)
	     write(7777,*)'Number of poloidal mode diff.(m1-m2) =',2*klim+1
		 write(7777,*)'Number of A (k//) per element =',2*(modva2-modva1)+1
	     if (READ_TABLES .eq. .FALSE.) then
		    write(7777,*)'Method : Calculating M12 values explicitly'
	     else
		    write(7777,*)'Method : Interpolating M12 values from tables'
	     end if
	   end if

	   if (WRITE_SCREEN) then
	     print *,'====================================================='
	     print *,'Calculation of plasma response M12 for k// = constant'
	     print *,'====================================================='
	     print *
...
c       1) Case of constant k// coordinates -------------------------------------
	    ...
	    tempo0 = etime (T1)
	    print *, '		 --> (ASPLAS.f) Begin FOUGDR + M12COKPCO:', tempo0

c         1.1) Create folder for data output at radial point: '../../M12run/rXXXX/'    
		if (WRITE_OUTPUT) then
	       call int_to_string4 (nint(1000*abscis(intab)), rrr)	
	       COKFOLDER = '../../M12run/r' // rrr
	       shell_comm = 'mkdir ..\..\M12run\r' // rrr
	       resp = SYSTEM(shell_comm)
	       if (resp .eq. -1) then
	          print *, 'Error creating folder: ', COKFOLDER
		      stop
	       end if
		   write(5555,'(i8, g15.6, A20)'), intab, abscis(intab), COKFOLDER
c	       print *, intab, abscis(intab), COKFOLDER
	    end if  ! WRITE_OUTPUT = true
...
c		     implemented, because Max. and non-Max. need different geom. coef.
c		     NEW: If reading M12 table files we don't need this call!!
		if (READ_TABLES .eq. .FALSE.) then
cERN			call fougdr(1)
			call fougdr_ern(1)	! fougdr_ern: vectorized version (checked OK!)
		else
		    if (WRITE_OUTPUT) then
		        shell_comm2 = 'copy ..\..\M12tables\r' // rrr // '\Plm.dat ' 
     ;                            // '..\..\M12run\r' // rrr 
				resp = SYSTEM(shell_comm2)
			end if
		end if
...
cERN		      k_index = mav2-2*m1+1
		...
		tempo = etime (T1)
		print *, '		 --> (ASPLAS.f)   End FOUGDR + M12COKPCO:', tempo
...
cPL@          to do: allow evaluation 1 species at a time:
              print *,'ASPLAS gdr: must implement calc. 1 species at a time'
	        stop
		 end if
...
c	  1) Case of constant k// coordinates -----------------------------------
        ...
        if(cokpco) then        
	     ...
	     tempo0 = etime (T1)
c	     print *, '		---> (ASPLAS.f) Begin FOUGDR + M12COKPCO:', tempo0
...
cERN		       k_index = mav2-2*m1+1
	           ...
	           vmat(5,5,iplb) = m12landau(k_index, m_index) ! p = 0 
cERN 02Feb2005
c		if(abs(k).eq.3)then
c			vmat(1,1,iplb) = vmat(1,1,iplb) * 0.86d0
c			vmat(3,3,iplb) = vmat(3,3,iplb) * 0.081d0
c		end if
c	    if(abscis(intab)>0.3) then
c			write(11119,*), iplb, m_index, k, dreal(vmat(1,1,iplb))
c		end if
...
c       Species with a general dielectric response:
           if(onespe)then
cPL@          to do: allow evaluation 1 species at a time:
              print *,'ASPLAS gdr 2nd call: must implement calculation
     ;                 1 species at a time'
	        stop
           end if
c          Compute x, v meshes at current radius and interpolate equil. distr. f0:
