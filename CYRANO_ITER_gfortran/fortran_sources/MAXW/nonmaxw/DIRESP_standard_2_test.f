      SUBROUTINE DIRESP_standard_2_test(isp)
...
c	For special x-grid
	...
	real*8 :: DELXF(500)
	integer :: Nsepp, Nsept, Ncplx, Ntan1, Ntan2, Nroot(lxg+400)

	real*8 :: treact2(1:2*nmoant-1,0:Nchi,-1:1), xngaugx(lxg+400), 
     ;          ICroots(lxg+400,3)
...
c	  sigma = +1
	  call insert_points2(Nxx, xngaugx(1:Nxx), xnsep, 'R', 2, 4, 2.d-4, xngaugx(1:Nxx+4))
	  Nxx = Nxx + 4
	  call insert_points2(Nxx, xngaugx(1:Nxx), xnsep, 'L', 2, 8, 2.d-4, xngaugx(1:Nxx+12))	
	  Nxx = Nxx + 8

c	  sigma = -1
	  call insert_points2(Nxx, xngaugx(1:Nxx), -xnsep, 'R', 2, 8, 2.d-4, xngaugx(1:Nxx+12))
	  Nxx = Nxx + 8
	  call insert_points2(Nxx, xngaugx(1:Nxx), -xnsep, 'L', 2, 4, 2.d-4, xngaugx(1:Nxx+4))
	  Nxx = Nxx + 4
...
cx	 dx = xmax/(N_tab-1)
cx       do k = 1, N_tab
cx          xvec_chi(k) = (k-1) * dx   
cx       end do

cx	call thetamax_CHI(N_tab,xvec_chi)
...
c	   i_xtang = isrchfge(Nfine, xnfine(1:Nfine), 1, xntang)
	   ...
	   if(i_xtang > 3)then
	      call insert_points2(Nfine, xnfine(1:Nfine), xntang, 'L', 2, 5, 1.d-5, xnfine(1:Nfine+5))
	      Nfine = Nfine + 5
 	   end if
...
c	   i_xtang2 = isrchfge(Nfine, xnfine(1:Nfine), 1, xntang2)
	   ...
	   if(i_xtang2<Nfine-0)then
	      call insert_points2(Nfine, xnfine(1:Nfine), xntang2, 'R', 2, 5, 1.d-5, xnfine(1:Nfine+5))
	       Nfine = Nfine + 5
	   end if
...
ccccccccccccccc TESTING cccccccccccccccccccc
           ...
           if(x.le.xsep)then ! PASSING (max. 1 root) ----------------------------------------------
          


c              call find_IC_roots(qom(ispe), v, x, mave, ICroots(ix,1:3), Nroot(ix))
              call find_IC_roots_onesig(qom(ispe), v, x, sigma, mave, ICaux(ix,1:2), Nraux(ix))
  		  if(im.eq.ind_k)then
                  write(1555,"(10G20.8)")v, xn, ICaux(ix,1:2), Nraux(ix)
	        end if             

c              call orbit_integ_pass_std(x, v, sigma, mave, NMM, qom(ispe), auxtang, xtir1,
c     ;                                       testaux, testreac(ix,1:NMM), testact(ix,1:NMM), www)
	        raiz = ICaux(ix,1)
              call oi_pass_act(x, v, sigma, mave, NMM, qom(ispe), auxtang, xtir1,
     ;                                         raiz, testact(ix,1:NMM), www)
...
c		  call orbit_integ_trap_std(x, v, sigma, mave, NMM, qom(ispe), auxtang, 
c     ;                           xtir1, testaux, testreac(ix,1:NMM), testact(ix,1:NMM),www)	  
     ...
     ;                                         ICaux(ix,2), ICaux(ix,1), testact(ix,1:NMM), www)
           ...
           elseif(x.ge.xstar .and. x.le.xmax)then  ! TRAPPED (complex)  -------------------------------



c             call orbit_integ_trap_cplx_std(x, v, sigma, mave, NMM, qom(ispe), 
c     ;                                   testaux, testreac(ix,1:NMM), www)
	
					   
              testact(ix,1:NMM) = 0.d0   ! Always non-resonant
c			ICroots(ix,1:3)=0.d0
c			Nroot(ix) = 0
			if(im.eq.ind_k)then 
			      write(1554,"(10G20.8)")v, xn, 0.0d0, 0.0d0, 0
	                  write(1555,"(10G20.8)")v, xn, 0.d0, 0.d0, 0
                        write(1553,"(400G16.8)") x, xn, 0, 0, 0, 0
	        end if    
	...
	if(im.eq.ind_k)then
		...
		write(1550,*)v,xn,2.d0*NOVO*v*testact(ix,ind_mdiff)

		write(1551,*)v,xn,2.d0*NOVO*v*testaux, 
     ;                      2.d0*NOVO*v*testreac(ix,ind_mdiff)
...
c	testreac(ix,1:NMM) =  -2.d0*NOVO*v*testreac(ix,1:NMM)
...
c        if(ix .gt. 1)then
c           treact2test(im,1:NMM) = treact2test(im,1:NMM) 
c     ;     + 0.5d0*(   testreac(ix,1:NMM) *   (v*v*v*dfdv-2.d0*v*v*(x-xtir1)*dfdx) 
c     ;               + testreac(ix-1,1:NMM) * (v*v*v*dfdv_prev-2.d0*v*v*(xprev-xtir1)*dfdx_prev)) 
...
c     ;			  BUFFERED='YES', BLOCKSIZE = 32*512, BUFFERCOUNT = 50, 
     ...
     ;                                                treact2test(k,1:NMM)/fac, tact2test(k,1:NMM)/fac
				  end do 
			close (13)
