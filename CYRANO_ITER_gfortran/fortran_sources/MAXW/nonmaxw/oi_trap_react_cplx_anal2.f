      SUBROUTINE oi_trap_react_cplx_anal2(xin, vin, sigm, mav, Nmdiff, qovm, 
     ;                                   testanal, FLAG)
...
c				if(j.eq.1)then
c				meuteste(ipo) = dcos(mdiff*thref) * ( coefA(1) + coefA(2) * (th(ipo)-thref) ) 
...
c      if(FLAG)then
c         write(1559,"(1000G16.8)"), xin, xnout, NTT, 0.d0, 0.d0, meuteste(1:NTT)
c         write(1559,"(1000G16.8)"), xin, xnout, NTT, 0.d0, 0.d0, th(1:NTT)
c      end if


      return
 
      END SUBROUTINE oi_trap_react_cplx_anal2