...
ccccccccccccccccccccc LUENNE ccccccccccccccccccccc
      double precision tempo_total, teste 
...
c     variavel para contagem de tempo	  
c	  tempo_total = 0.0
      open (arquivo_tempo, FILE = 'TEMPO.dat')
		call powabs(i, onespe, ispe, .false., tempo_total)
c		tab(,,1) to give absorption to order 0 in lr;
c		tab(,,2) to give absorption up to 2nd order in lr


ccccccccccccccccccccccccc Imprime tempo ccccccccccccccccccccccccc    
         write (arquivo_tempo,*)"Tempo powabs = ", tempo_total
	     close (arquivo_tempo)
